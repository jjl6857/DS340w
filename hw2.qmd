...
# 0 row drop since it still got 4177 row like the frame abalone
```
...
ggplot(df_long, aes(x = value)) + 
  ...
  theme_minimal() + 
  labs(title = "Histograms of Quantitative Variables", y = "Count")
```
...
ggplot(df, aes(x=sex, y=diameter)) +
  ...
  theme_minimal()
```
# The shape of the violin plot can reveal differences in the distribution of diameters. Wider sections of the violin plot represent a higher frequency of data points.
...
ggplot(df,aes(x = length, y = diameter)) +
  geom_point(aes(color = sex, size = shell_weight))  +
  theme_minimal()
```
...
ggplot(df,aes(x = length, y = diameter)) +
  ...
  geom_smooth(method = "lm", se = FALSE, aes(linestyle = sex)) +
  facet_wrap(~ sex)
  theme_minimal()
```
...
```{R}
ggplot(long_means, aes(x = variable, y = mean_value, fill = sex)) +
...
```{R}
ggplot(avg_weight_df, aes(x = sex, y = num_rings, fill = avg_weight)) +
  geom_tile() +
...
```{R}
quantitative_vars <- c(colnames(new_df))
quantitative_vars
```
```{R}
plots <- map2(quantitative_vars, rep("rings", length(quantitative_vars)), ~ {
  ggplot(new_df, aes_string(x = .y, y = .x, color = "sex")) +
    geom_point() +
...
```{R}
linear_model <- lm(height ~ diameter, data = new_df)
...
```{R}
plot(new_df$diameter, new_df$height ,xlab = "Diameter", ylab = "Height", pch = 19, col = 'blue')
...
```{R}
new_heights <- predict(linear_model, newdata = data.frame(diameter = new_diameters))
new_heights
```
```{R}
plot(new_df$diameter, new_df$height ,xlab = "Diameter", ylab = "Height", pch = 19, col = 'blue')
